...
let transacciones: Transaccion[] = [ {
  id: 'e39af8db-aa87-4f57-a94e-7d89fc8bf84d', tipo: TipoTransaccion.Ingreso, fecha: new Date(), concepto: 'Salario 1', monto: 400,
}, {
  id: '5ab4cace-a63e-42e2-aec6-2ca2051f0e43', tipo: TipoTransaccion.Ingreso, fecha: new Date(), concepto: 'Salario 2', monto: 400,
}, {
  id: 'c7e08b05-d0f1-4e22-b999-16a60f397e80', tipo: TipoTransaccion.Egreso, fecha: new Date(), concepto: 'Supermercado', monto: 200,
}, {
  id: '8b793047-33f8-4f79-a57d-538d2485493e', tipo: TipoTransaccion.Egreso, fecha: new Date(), concepto: 'Servicios básicos', monto: 100,
} ];
...
export const getEgresosTotal = async(): Promise<number> => {
  ...
  return Number(Number(totalEgresos * -1).toFixed(2));
}
...
export const getEgresosTotalPorcentaje = async() => {
  const ingresos = await getIngresosTotal();
  const egresos = await getEgresosTotal();
  const porcentaje = ((egresos * 100)/ ingresos) * -1;
